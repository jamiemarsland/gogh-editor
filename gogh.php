...
add_action( 'wp_enqueue_scripts', function () {
	...
	wp_register_script( 'gogh-compose', plugins_url( 'gogh-compose.js', __FILE__ ), array(), '0.99.238-chrome', true );
	wp_enqueue_script( 'gogh-write', plugins_url( 'gogh-write.js', __FILE__ ), array( 'gogh-compose' ), '0.99.238-chrome', true );
	wp_enqueue_style( 'gogh-write', plugins_url( 'gogh-write.css', __FILE__ ), array(), '0.99.238-chrome' );
...
function gogh_motion_css() {
	...
	return
		...
		// CALM — a soft fade with barely-there lift; over before you notice
		$targets( 'calm' ) . ' { animation: gogh-mo-calm linear both; animation-timeline: view(); animation-range: entry 5% entry 45%; }' .
		'@keyframes gogh-mo-calm { from { opacity: 0; translate: 0 10px; } }' .
		// RISE — content lifts into place with a small sibling stagger
		$targets( 'rise' ) . ' { animation: gogh-mo-rise linear both; animation-timeline: view(); animation-range: entry 0% entry 50%; }' .
		'html:not(.gogh-editing) body.gogh-motion-rise .gogh-section > :nth-child(2) { animation-range: entry 8% entry 58%; }' .
		'html:not(.gogh-editing) body.gogh-motion-rise .gogh-section > :nth-child(3) { animation-range: entry 16% entry 66%; }' .
		'html:not(.gogh-editing) body.gogh-motion-rise .gogh-section > :nth-child(n+4) { animation-range: entry 24% entry 74%; }' .
		'@keyframes gogh-mo-rise { from { opacity: 0; translate: 0 32px; } }' .
		// DRAMA — bigger travel and a breath of scale
		$targets( 'drama' ) . ' { animation: gogh-mo-drama linear both; animation-timeline: view(); animation-range: entry 0% entry 65%; }' .
		'@keyframes gogh-mo-drama { from { opacity: 0; translate: 0 56px; scale: 0.97; } }' .
		'} }';
}
...
add_action( 'rest_api_init', function () {
	...
	register_rest_route( 'gogh/v1', '/version', array(
		...
		'callback'            => function () {
			return array( 'build' => '0.99.238-chrome' );
		},
	) );
...
add_action( 'wp_enqueue_scripts', function () {
	...
	wp_register_style( 'gogh-base', false, array(), '0.99.238-chrome' );
	...
	wp_register_script( 'gogh-view', false, array(), '0.99.238-chrome', true );
	...
	wp_register_script( 'gogh-compose', plugins_url( 'gogh-compose.js', __FILE__ ), array(), '0.99.238-chrome', true );
	wp_enqueue_script( 'gogh-editor', plugins_url( 'gogh-editor.js', __FILE__ ), array( 'gogh-compose' ), '0.99.238-chrome', true );
	wp_enqueue_style( 'gogh-editor', plugins_url( 'gogh-editor.css', __FILE__ ), array(), '0.99.238-chrome' );
	...
	if ( isset( $_GET['gogh-mcp'] ) || $gogh_exp || apply_filters( 'gogh_webmcp_enabled', false ) ) {
		wp_enqueue_script( 'gogh-webmcp', plugins_url( 'gogh-webmcp.js', __FILE__ ), array( 'gogh-editor' ), '0.99.238-chrome', true );
	}
	...
	if ( isset( $_GET['gogh-test'] ) ) {
		wp_enqueue_script( 'gogh-tests', plugins_url( 'gogh-tests.js', __FILE__ ), array( 'gogh-editor' ), '0.99.238-chrome', true );
	}
...
add_action( 'enqueue_block_assets', function () {
	...
	wp_register_style( 'gogh-editor-base', false, array(), '0.99.238-chrome' );
