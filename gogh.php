...
add_action( 'wp_enqueue_scripts', function () {
	...
	wp_enqueue_script( 'gogh-write', plugins_url( 'gogh-write.js', __FILE__ ), array(), '0.99.95-chrome', true );
	wp_enqueue_style( 'gogh-write', plugins_url( 'gogh-write.css', __FILE__ ), array(), '0.99.95-chrome' );
...
add_action( 'wp_enqueue_scripts', function () {
	...
	wp_register_style( 'gogh-base', false, array(), '0.99.95-chrome' );
	wp_register_script( 'gogh-view', false, array(), '0.99.95-chrome', true );
	...
	wp_enqueue_script( 'gogh-editor', plugins_url( 'gogh-editor.js', __FILE__ ), array(), '0.99.95-chrome', true );
	wp_enqueue_style( 'gogh-editor', plugins_url( 'gogh-editor.css', __FILE__ ), array(), '0.99.95-chrome' );
	...
	if ( isset( $_GET['gogh-mcp'] ) || $gogh_exp || apply_filters( 'gogh_webmcp_enabled', false ) ) {
		wp_enqueue_script( 'gogh-webmcp', plugins_url( 'gogh-webmcp.js', __FILE__ ), array( 'gogh-editor' ), '0.99.95-chrome', true );
	}
	...
	if ( isset( $_GET['gogh-test'] ) ) {
		wp_enqueue_script( 'gogh-tests', plugins_url( 'gogh-tests.js', __FILE__ ), array( 'gogh-editor' ), '0.99.95-chrome', true );
	}
...
add_action( 'enqueue_block_assets', function () {
	...
	wp_register_style( 'gogh-editor-base', false, array(), '0.99.95-chrome' );
...
add_action( 'admin_bar_menu', function ( $bar ) {
	...
	if ( current_user_can( 'publish_pages' ) ) {
		$bar->add_node( array(
			'id'    => 'gogh-new-page',
			'title' => '<svg class="gogh-ab-ic" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg>New page',
			'href'  => wp_nonce_url( admin_url( 'admin-post.php?action=gogh_new_page' ), 'gogh_new_page' ),
		) );
	}
	// ✍ Write — only when the site has somewhere for posts to live
	$has_blog = 'posts' === get_option( 'show_on_front' ) || (int) get_option( 'page_for_posts' ) > 0;
	if ( $has_blog && current_user_can( 'edit_posts' ) ) {
		$bar->add_node( array(
			'id'    => 'gogh-write',
			'title' => '<svg class="gogh-ab-ic" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>New post',
