...
add_action( 'wp_enqueue_scripts', function () {
	...
	wp_enqueue_script( 'gogh-write', plugins_url( 'gogh-write.js', __FILE__ ), array(), '0.99.99-chrome', true );
	wp_enqueue_style( 'gogh-write', plugins_url( 'gogh-write.css', __FILE__ ), array(), '0.99.99-chrome' );
...
add_action( 'wp_enqueue_scripts', function () {
	...
	wp_register_style( 'gogh-base', false, array(), '0.99.99-chrome' );
	wp_register_script( 'gogh-view', false, array(), '0.99.99-chrome', true );
	...
	wp_enqueue_script( 'gogh-editor', plugins_url( 'gogh-editor.js', __FILE__ ), array(), '0.99.99-chrome', true );
	wp_enqueue_style( 'gogh-editor', plugins_url( 'gogh-editor.css', __FILE__ ), array(), '0.99.99-chrome' );
	...
	if ( isset( $_GET['gogh-mcp'] ) || $gogh_exp || apply_filters( 'gogh_webmcp_enabled', false ) ) {
		wp_enqueue_script( 'gogh-webmcp', plugins_url( 'gogh-webmcp.js', __FILE__ ), array( 'gogh-editor' ), '0.99.99-chrome', true );
	}
	...
	if ( isset( $_GET['gogh-test'] ) ) {
		wp_enqueue_script( 'gogh-tests', plugins_url( 'gogh-tests.js', __FILE__ ), array( 'gogh-editor' ), '0.99.99-chrome', true );
	}
...
add_action( 'enqueue_block_assets', function () {
	...
	wp_register_style( 'gogh-editor-base', false, array(), '0.99.99-chrome' );
...
add_action( 'admin_bar_menu', function ( $bar ) {
	...
	} elseif ( ! $is_post_room && current_user_can( 'publish_pages' ) ) {
		$bar->add_node( $new_page );
	}
	...
	// the site menu holds the SITE-scoped act: Design site opens the front
	// page with the style drawer already open. Matching scope and location
	// is what keeps it un-confusing — the bar stays contextual.
	if ( current_user_can( 'edit_theme_options' ) ) {
		$pal2 = '<svg class="gogh-ab-ic" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><circle cx="13.5" cy="6.5" r="2.1"/><circle cx="18.5" cy="12" r="2.1"/><circle cx="8.5" cy="7.5" r="2.1"/><circle cx="6" cy="13" r="2.1"/><path d="M12 22a10 10 0 1 1 10-10c0 2.2-1.8 3-3.5 3H16a2.5 2.5 0 0 0-2 4c.5.7 0 3-2 3z"/></svg>';
		$bar->add_node( array(
			'id'     => 'gogh-design-site',
			'parent' => 'site-name',
			'title'  => $pal2 . 'Design site',
			'href'   => add_query_arg( array( 'gogh-edit' => '1', 'gogh-design' => '1' ), home_url( '/' ) ),
		) );
	}
