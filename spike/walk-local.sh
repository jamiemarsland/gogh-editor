...
# login cookies and lands logged out — use a fresh port per run. And the
# auto-login is ONE SHOT: the first request gets it, so do not probe the
# server with curl before the browser — wait for "Ready!" in its output.
PORT="${1:-9400}"
cd "$(dirname "$0")/.." || exit 1
